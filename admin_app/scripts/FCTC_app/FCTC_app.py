...
class Main_window(QMainWindow):
    ...
    def init_UI(self):
        ...
        self.table      = QTableView(self.da_page)
...
drive"                                        : None,
 ...
 control mode."                              : None,
        ...
        self.show_table_button                = self.make_button("Show table", 350, 700, parent=self.da_page)
    ...
    def on_approved_state_change(self):
        ...
        if not (self.user_type == "admin" or self.user_type == "approver"):
            ...
            if self.approved_checkbox.isChecked():
                ...
                self.approved_by_name.setText("No name found")
                self.approved_by_date.setText("No date found")
                self.approved_by_name.adjustSize()
                self.approved_by_date.adjustSize()
                self.show_signature_image(object=self.approved_by_signature,no_sig=True)
    ...
    def show_table(self):
        ...
        table_data = DataFrame(
            table_data,
            columns=['actuator serial number', "assembled by", "tested by", "approved by"]
        )
